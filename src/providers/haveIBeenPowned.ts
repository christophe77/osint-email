...
const haveIBeenPowned = async (email: string) => {
	const formattedEmail = email.replace('@', '%40');
	try {
		const results = await axiosInstance.get(
			`https://haveibeenpwned.com/unifiedsearch/${formattedEmail}`,
		);
		return results;
	} catch (error: any) {
		return {
...
const datas = {
	Breaches: [
		...
		},
		{
			...
			LogoPath:
				'https://haveibeenpwned.com/Content/Images/PwnedLogos/Malwarebytes.png',
			DataClasses: [
				'Dates of birth',
				'Email addresses',
				'IP addresses',
